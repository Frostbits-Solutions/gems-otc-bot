...
export interface WebAppInitData {
  ...
  /**
   * Optional. An object containing data about the chat partner of the current user
  ...
  /**
   * Optional. Global identifier, uniquely corresponding to the chat from which
   * the Mini App was opened.
   ...
   * GET-parameter tgWebAppStartParam, so the Mini App can load the correct interface right away.
   */
  start_param?: string
  ...
  /**
   * A signature of all passed parameters (except hash),
   * which the third party can use to check their validity.
   */
  signature?: string
}
