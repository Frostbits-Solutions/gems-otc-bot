...
<template>
  ...
  >
      ...
      <div class="grid grid-cols-[auto_auto_auto] items-center gap-5">
        ...
        <div class="flex items-baseline gap-1">
          <span class="text-lg font-bold text-white">{{
            pair.base_asset.ticker
          }}</span>
          <span class="text-xs font-medium text-gray-500">/</span>
          <span class="text-sm font-medium text-gray-400">{{
            pair.quote_asset.ticker
          }}</span>
        </div>

        <div
          class="p-[1px] rounded-lg bg-gradient-to-r from-brand-primary to-brand-secondary"
        >
          <UButton
    ...
    <div class="grid grid-cols-3 gap-3">
      ...
      <div class="flex flex-col gap-0.5">
        ...
        >
        <span
          class="text-base font-bold text-gray-200 tracking-tight tabular-nums"
        >
          {{ formatVolume }}
        </span>
      </div>
  ...
  >
    ...
    <div class="flex items-center flex-shrink-0 w-[70px] ml-1">
      <span class="text-[13px] font-bold text-gray-100">{{
        pair.base_asset.ticker
      }}</span>
