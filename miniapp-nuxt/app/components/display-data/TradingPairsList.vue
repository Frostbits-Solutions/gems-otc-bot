...
const getPairKey = (pair: TradingPair) =>
  `${pair.base_asset.asa_id}-${pair.quote_asset.asa_id}`;
...
<template>
  <div class="flex flex-col flex-grow">
    ...
    >
      <UIcon
        name="i-heroicons-arrow-path"
        class="w-10 h-10 text-[#667eea] animate-spin mb-3"
      />
    ...
    >
      <p
        class="text-sm text-[var(--tg-theme-hint-color,#9ca3af)] font-medium mb-3"
      >
        {{ store.error }}
      </p>
      <UButton
    ...
    <div v-else class="flex flex-col gap-3 flex-grow items-center py-8 w-full">
      ...
      >
        <p
          class="text-sm text-[var(--tg-theme-hint-color,#9ca3af)] font-medium"
        >
