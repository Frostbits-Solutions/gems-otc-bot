...
<template>
  <div class="flex flex-col gap-4 w-full max-w-md mx-auto animate-fade-in">
    ...
    <DisplayDataTradePair
      :pair="pair"
      mode="header"
      @swap="emit('swap-assets')"
    />

    <div
      class="flex flex-col gap-0 shadow-sm overflow-hidden rounded-2xl border border-white/10"
    >
      <!-- Receive Container -->
      <SwapAssetContainer
    ...
    <div
      v-if="isOverride && pair.current_price !== null"
    ...
    <div
      ...
      :class="
        isValidOrder ? '' : 'bg-neutral-950 border border-white/5 shadow-inner'
      "
    >
